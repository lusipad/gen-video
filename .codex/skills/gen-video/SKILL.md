...
这个 skill 不再把自己当成“固定 prompt 模板库”，而是一个更薄的 `编排层 + 约束层 + 评测层`，并由独立知识层持续供给最新判断。
...
- `github-issue-inbox.json` 和 `issue-inbox.md` 负责把 GitHub issue 收藏夹接入知识闭环
