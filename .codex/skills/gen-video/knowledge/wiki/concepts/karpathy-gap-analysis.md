...
这个项目的前台定位已经很明确：它是一个 `AI 视频导演技能`。

Karpathy 风格的 `raw knowledge base + LLM wiki` 思路，在这里主要被拿来构建“导演后台”，让前台判断持续校准，但还没有完全走到 Karpathy 那种形态。
...
这些已经让 skill 从“静态 prompt 模板”转成了“带持续学习后台的导演技能”。
...
`它的前台已经是 AI 视频导演技能，后台则是一个更保守、保留人工闸门的 Karpathy 风格学习系统。`
...
- 判断 skill 什么时候该退后
- 用知识层而不是对话上下文保存判断
- 让新的模型、新的经验帖、新的失败案例能持续进入同一个导演后台
