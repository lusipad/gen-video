...
`knowledge/` 是 `gen-video` 的长期记忆层，参考 Andrej Karpathy 提出的 `raw knowledge base + LLM wiki` 思路组织。
...
6. `build_query_writeback_queue.py` 把 recurring question 变成 backlog
7. 维护者据此更新 `wiki/`、`profiles/`、`benchmarks/` 或 `core/`
8. 再把动作记录进 `log.md`
...
这个 CI 默认不改写 `wiki/` 的人工判断，只负责 `watchlist + nightly review + LLM review + issue inbox + lint + suggestion + writeback` 七层自动信号。
